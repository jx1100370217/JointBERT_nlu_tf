...
@software: pycharm
@file: run_intent_slot_jion_task_bert.py
@time: 2021/8/2 15:08
@desc: 意图槽位联合任务模块
'''
...
def model_fn_builder(bert_config, num_slot_labels, num_intent_labels, init_checkpoint,
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            intent_accuracy = tf.metrics.accuracy(intent_label_ids, intent_predictions)
            ...
            logging_hook = tf.train.LoggingTensorHook(
                {"steps": tf.train.get_or_create_global_step(),
                 ...
                 "slot_loss": slot_loss,
                 "intent_accuracy":intent_accuracy[1],
            ...
            output_spec = tf.estimator.EstimatorSpec(
                mode=mode,
                loss=total_loss,
                eval_metric_ops=metric_fn(intent_per_example_loss, intent_label_ids, intent_logits,
                             slot_per_example_loss, slot_label_ids, slot_logits, is_real_example))
        else:
            output_spec = tf.estimator.EstimatorSpec(
                mode=mode,
