...
from utils.model.bert import modeling
from utils.model.bert import optimization_hvd as optimization
from utils.model.bert import tokenization
import tensorflow as tf
import horovod.tensorflow as hvd
from config.config import bert_base, MAX_SEQ_LENGTH
from utils import calculate_model_score as tf_metrics
...
premodel_path = bert_base

## ab test parameters
tf.flags.DEFINE_string("preload", "", "gunicorn workers' number")
tf.flags.DEFINE_string("workers", "", "gunicorn workers' ip and port")
tf.flags.DEFINE_string("k", "", "gunicorn workers' config address")
tf.flags.DEFINE_string("bind", "", "gunicorn workers' config address")
tf.flags.DEFINE_string("log-level", "", "gunicorn workers' config address")
...
flags.DEFINE_string(
    "bert_config_file", os.path.join(premodel_path, 'bert_config.json'),
    "The config json file corresponding to the pre-trained BERT model. "
    "This specifies the model architecture.")

flags.DEFINE_string("task_name", None, "The name of the task to train.")

flags.DEFINE_string("vocab_file", os.path.join(premodel_path, 'vocab.txt'),
...
flags.DEFINE_string(
    "init_checkpoint", os.path.join(premodel_path, 'bert_model.ckpt'),
...
def convert_single_example(ex_index, example, slot_label_list,  max_seq_length, tokenizer):
...
def file_based_convert_examples_to_features(
        examples, slot_label_list, max_seq_length, tokenizer, output_file):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        feature = convert_single_example(ex_index, example, slot_label_list, max_seq_length, tokenizer)
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
                 slot_label_ids, num_slot_labels, use_one_hot_embeddings):
    ...
    return (slot_loss, slot_per_example_loss, slot_logits, slot_predictions)


def model_fn_builder(bert_config, num_slot_labels, init_checkpoint,
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            train_op = optimization.create_optimizer(
                slot_loss, learning_rate, num_train_steps, num_warmup_steps, use_tpu)

            slot_predictions = tf.argmax(slot_logits, axis=-1, output_type=tf.int32)
            slot_accuracy = tf.metrics.accuracy(slot_label_ids, slot_predictions)

            logging_hook = tf.train.LoggingTensorHook(
                {"steps": tf.train.get_or_create_global_step(),
                 "slot_loss": slot_loss,
                 "slot_accuracy": slot_accuracy[1]}, every_n_iter=50)
            ...
            def metric_fn(slot_per_example_loss, slot_label_ids, slot_logits, is_real_example):
            ...
            output_spec = tf.estimator.EstimatorSpec(
                mode=mode,
                loss=slot_loss,
                eval_metric_ops=metric_fn(slot_per_example_loss, slot_label_ids, slot_logits, is_real_example))
        else:
            output_spec = tf.estimator.EstimatorSpec(
                mode=mode,
                predictions={"slot_predictions": slot_predictions})
        return output_spec
...
def main(_):
    ...
    if FLAGS.do_train:
        ...
        if not os.path.exists(train_file):
            file_based_convert_examples_to_features(
                train_examples, slot_label_list, FLAGS.max_seq_length, tokenizer, train_file)
    ...
    if FLAGS.do_eval:
        ...
        if not os.path.exists(eval_file):
            file_based_convert_examples_to_features(
                eval_examples, slot_label_list, FLAGS.max_seq_length, tokenizer, eval_file)
        ...
        with tf.gfile.GFile(output_eval_file, "w") as writer:
            ...
            for key in sorted(result.keys()):
                ...
                writer.write("%s = %s\n" % (key, str(result[key])))
                eval_res.append(f"{key} : {str(result[key])}")
    ...
    if FLAGS.do_predict:
        ...
        file_based_convert_examples_to_features(predict_examples, slot_label_list,FLAGS.max_seq_length,
        ...
        with tf.gfile.GFile(slot_output_predict_file, "w") as slot_writer:
            num_written_lines = 0
            tf.logging.info("***** Slot Filling results *****")
