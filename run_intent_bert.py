        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            logging_hook = tf.train.LoggingTensorHook(
                {"steps": tf.train.get_or_create_global_step(),
                 ...
                 "accuracy": accuracy[1]}, every_n_iter=50)
...
def main(_):
    ...
    if FLAGS.do_eval:
        ...
        for prediction, example in zip(dev_result, eval_examples):
            pred = label_list[np.argmax(prediction)]
            info = '{}\t{}\t{}'.format(example.text_a, example.label, pred)
            infos.append(info)
        with open(os.path.join(FLAGS.output_dir, "results.csv"), 'w',
